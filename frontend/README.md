...
Truy cập `http://localhost:5173/admin` hoặc `http://localhost:5173/admin/login`, đăng nhập bằng tài khoản ADMIN.
...
| Sản Phẩm | `GET /api/admin/products`, xóa qua `DELETE /api/admin/products/{id}` |
...
Khi backend bổ sung các endpoint trên, chỉ cần thêm file mới trong `src/admin/api/` theo đúng pattern các
