...
function VoucherForm({ initialValue, onClose, onSaved }) {
  const [form, setForm] = useState(() =>
    initialValue
      ? {
          ...
          minOrderAmount: initialValue.minOrderAmount || '0',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (initialValue?.id) {
        await vouchersApi.update(initialValue.id, payload)
      } else {
        await vouchersApi.create(payload)
      }
      onSaved()
    } catch (err) {
      setError(err.response?.data?.message || 'Lưu mã giảm giá thất bại.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-slate-800">{initialValue?.id ? 'Cập nhật mã giảm giá' : 'Thêm mã giảm giá'}</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Mã</label>
            <input
              required
              value={form.code}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Giới hạn lượt dùng</label>
            <input
              type="number"
              min="0"
...
export default function VoucherTab() {
  ...
  const fetchVouchers = useCallback(() => {
    ...
    vouchersApi
      ...
      .then(setData)
      .catch(() => setError('Không thể tải mã giảm giá từ backend.'))
  ...
  const handleDelete = async (id) => {
    if (!confirm('Xóa mã giảm giá này? Hành động không thể hoàn tác.')) return
    try {
      await vouchersApi.remove(id)
      fetchVouchers()
    } catch (err) {
      alert(err.response?.data?.message || 'Xóa mã giảm giá thất bại.')
    }
  }
    ...
    <div className="space-y-5">
          ...
          <tbody>
            {loading ? (
              ...
              data.items.map((voucher) => (
                <tr key={voucher.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                  <td className="p-4 font-semibold text-slate-700 whitespace-nowrap">{voucher.code}</td>
                  <td className="p-4">
                    <p className="font-medium text-slate-700">{voucher.name}</p>
                    <p className="text-xs text-slate-400 max-w-[220px]">{voucher.description || 'Không có mô tả'}</p>
                  </td>
                  <td className="p-4 text-slate-600 whitespace-nowrap">{formatDiscount(voucher)}</td>
                  <td className="p-4 text-slate-600 whitespace-nowrap">{formatCurrency(voucher.minOrderAmount)}</td>
                  ...
                  <td className="p-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ${statusStyles[voucher.status]}`}>
                      {statusLabel[voucher.status] || voucher.status}
                    </span>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-3 text-slate-400">
                      <button onClick={() => setEditing(voucher)} className="hover:text-brand-teal transition-colors">
                        <Pencil size={16} />
                      </button>
                      <button onClick={() => handleDelete(voucher.id)} className="hover:text-red-500 transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
      ...
      {data.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-slate-500">
          <p>
            Trang {data.page + 1} / {data.totalPages} - Tổng {data.totalElements} mã
          </p>
      ...
      {editing && <VoucherForm initialValue={editing.id ? editing : null} onClose={() => setEditing(null)} onSaved={handleSaved} />}
    </div>
  )
}
