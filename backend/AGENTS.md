...
Auth token rule:
- Login phải trả `accessToken`, `refreshToken`, `tokenType` và `expiresIn`.
- Access token là JWT ngắn hạn và phải có claim `jti`.
- Refresh token là chuỗi random dài hạn hơn, database chỉ lưu hash, không lưu raw token.
- `POST /api/auth/refresh` rotate refresh token: revoke token cũ và cấp token mới.
- `POST /api/auth/logout` revoke refresh token và blacklist access token theo `jti`.
- `JwtAuthenticationFilter` phải kiểm tra blacklist trước khi set authentication.
- Bản lab dùng MySQL cho `refresh_tokens` và `blacklisted_tokens`, không dùng Redis.
